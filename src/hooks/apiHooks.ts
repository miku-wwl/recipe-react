...
const useRecipe = () => {
  ...
  const postRecipe = async (
    ...
    const recipe: Omit<MediaItem, 'media_id' | 'created_at'> = {
      user_id: user_id,
      thumbnail: file.data.filename,
      title: inputs.title,
    ...
    return await fetchData<MediaResponse>(
      import.meta.env.VITE_MEDIA_API + '/upload/media',
      options,
    );
  };
...
const useLike = () => {
  ...
  const getUserLike = async (
    getUserLikeRequest: GetUserLikeRequest,
    token: string,
  ) => {
  ...
  const getLikeListWithOwner = async (media_id: number) => {
    // send request to /media/:media_id to get like list by media
    const getLikeListByMediaId = await fetchData<Like[]>(
      import.meta.env.VITE_MEDIA_API + '/likes/media/' + media_id,
    );

    const likeListWithOwner = await Promise.all(
      getLikeListByMediaId.map(async (likeItem) => {
        const owner = await getUserById(likeItem.user_id);
        const likeItemWithOwner: LikeItemWithOwner = {
          ...likeItem,
          username: owner.username,
        };
...
const useComment = () => {
  ...
  const getCommentsByMediaId = async (media_id: number) => {
    ...
    const comments = await fetchData<Comment[]>(
      import.meta.env.VITE_MEDIA_API + '/comments/media/' + media_id,
    );
